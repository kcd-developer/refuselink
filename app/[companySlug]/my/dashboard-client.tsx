...
import {
  daysUntilService,
  serviceWeekLabel,
  type ServiceWeekCycle,
} from "@/lib/service-week-cycle";
...
export function CustomerDashboardClient({
  ...
  const allServices = (addressServices ?? []).flatMap(
    (item: any) => item.services ?? [],
  );
  const firstName = userName?.split?.(" ")?.[0] ?? "Customer";
  const primaryAccount =
    (accounts ?? []).find((account: any) => account?.isPrimary)?.customer ??
    (accounts ?? [])[0]?.customer;
  ...
  const dashboardServices = primaryServices.length
    ? primaryServices
    : allServices;
  const scheduledServices = dashboardServices.filter(
    (service: any) =>
      service.service === "trash" ||
  ...
  const upcomingServices = scheduledServices
    .map((service: any) => ({
      ...
      daysUntil: daysUntilService(
        currentDate,
        service.dayOfWeek,
        (service.weekCycle ?? null) as ServiceWeekCycle,
      ),
    }))
  ...
  const nextServices = nextService
    ? upcomingServices.filter(
        (service: any) => service.daysUntil === nextService.daysUntil,
      )
    : [];
  ...
  const hasTrashService = nextServices.some(
    (service: any) => service.service === "trash",
  );
  const hasRecyclingService = nextServices.some(
    (service: any) =>
      service.service === "recycling" || service.service === "recycle",
  );
  const yardWasteService = nextServices.find(
    (service: any) => service.service === "yard_waste",
  );
  ...
  return (
    <div className="space-y-6 pb-8">
      <section
        ...
        style={{
          background: `linear-gradient(118deg, ${primaryColor} 0%, #0849ad 62%, var(--company-secondary) 145%)`,
        }}>
        ...
        <div className="relative z-10 max-w-[62%]">
          <h1 className="font-display text-[clamp(1.875rem,3.2vw,2.25rem)] font-bold tracking-tight">
            Welcome, {firstName}
          </h1>
          <div className="mt-[clamp(3.5rem,5vw,4.25rem)]">
            <div>
              ...
              <p className="mt-1 font-display text-[clamp(1.875rem,4vw,3rem)] font-bold leading-none">
                {nextServiceLabel}
              </p>
            </div>
          </div>
        ...
        {hasTrashService || hasRecyclingService || hasYardWasteService ? (
          <div
            ...
            {hasTrashService && (
              <Image
                src="/images/kc-disposal-trash-cart-black-lid.png"
                alt=""
                width={520}
                height={500}
                priority
          ...
          <Leaf
            className="absolute -bottom-8 right-[clamp(1.25rem,5vw,3.5rem)] h-[clamp(13rem,24vw,16rem)] w-[clamp(13rem,24vw,16rem)] rotate-[-10deg] text-white/14"
            strokeWidth={1.2}
          />
        )}
      </section>
      ...
      <section className="grid grid-cols-1 gap-4 min-[700px]:grid-cols-3">
        {stats.map(({ label, value, icon: Icon, accent }) => (
          <div
            key={label}
            className="flex min-h-28 items-center gap-[clamp(0.75rem,1.8vw,1.25rem)] rounded-2xl border border-slate-200/80 bg-white px-[clamp(0.875rem,2vw,1.25rem)] py-4 shadow-[0_10px_28px_-20px_rgba(15,23,42,0.45)]">
            <span
              className="flex h-[clamp(3rem,5vw,3.5rem)] w-[clamp(3rem,5vw,3.5rem)] shrink-0 items-center justify-center rounded-full text-white shadow-lg ring-4 ring-slate-50"
            ...
            <div className="min-w-0">
              ...
              <p className="mt-2 whitespace-nowrap text-[clamp(0.8125rem,1.3vw,0.875rem)] text-slate-500">
                {label}
              </p>
            </div>
          </div>
      ...
      <section className="grid gap-6 md:grid-cols-2">
                  ...
                  {primaryServices.length ? (
                    primaryServices.map((service: any) => (
                      <span
                        ...
                        style={
                          serviceStyles[service.service] ?? serviceStyles.trash
                        }>
                        {serviceLabels[service.service] ?? service.service} ·{" "}
                        {dayNames[service.dayOfWeek]}
                        {service.weekCycle
                          ? ` · ${serviceWeekLabel(service.weekCycle)}`
                          : ""}
          ...
          <div className="mt-5 space-y-3">
            {paymentUrl && (
              <a
                ...
                className="group flex items-center justify-between rounded-xl border border-slate-200 px-3 py-3 transition hover:border-blue-200 hover:bg-blue-50/50">
            ...
            />
            <QuickAction
              href={`/${companySlug}/my/service-schedules`}
              label="View Schedule"
              icon={Calendar}
              color="#4D9221"
            />
            <QuickAction
              href={`/${companySlug}/my/documents`}
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_10px_30px_-22px_rgba(15,23,42,0.45)] md:col-span-2">
          <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5 sm:px-7">
            ...
            <Link
              href={`/${companySlug}/my/announcements`}
