...
export function EmployeeServiceHoldsClient({ companySlug, requests, suspendedAddresses }: { companySlug: string; requests: any[]; suspendedAddresses: any[] }) {
  ...
  async function restoreDirectly(address: any) {
    if (!window.confirm(`Confirm service was restored in Soft-Pak for ${address.address}?`)) return
    const note = window.prompt('Optional internal note (for example, who called):')
    if (note === null) return
