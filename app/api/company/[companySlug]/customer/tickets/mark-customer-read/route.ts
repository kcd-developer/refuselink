...
export async function POST(request: Request, { params }: { params: Promise<{ companySlug: string }> }) {
  ...
  const allowed = await prisma.ticket.findMany({
    where: { id: { in: parsed.data.ticketIds }, companyId: user.companyId, customer: { userAccess: { some: { customerUserId: user.id } } } },
    select: { id: true },
  })
  await prisma.customerTicketRead.createMany({ data: allowed.map((ticket) => ({ ticketId: ticket.id, customerUserId: user.id })), skipDuplicates: true })
